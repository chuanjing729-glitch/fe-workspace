---
layout: home

hero:
  name: "前端工程效率平台"
  text: "提升开发质量、效率和性能"
  tagline: 规范检查工具 · 完整规范文档 · AI编码助手 · 工具库
  actions:
    - theme: brand
      text: Webpack 插件
      link: /packages/webpack-spec-plugin/
    - theme: alt
      text: AI 编码助手
      link: /ai-guidelines/
    - theme: alt
      text: 查看规范
      link: /specs/
    - theme: alt
      text: 工具库
      link: /packages/core-utils/

features:
  - icon: 🔍
    title: Webpack 规范检查插件
    details: 7大检查维度、36项规则，集成在构建流程中自动检测代码质量问题
    link: /packages/webpack-spec-plugin/
    
  - icon: 📝
    title: 完整的规范体系
    details: 涵盖编码、工程化、设计、性能优化等多个维度，50+份详细文档
    link: /specs/
    
  - icon: 🛠️
    title: 前端工具库
    details: 提供 DOM 操作、数据校验、常用工具函数等
    link: /packages/core-utils/
    
  - icon: ✅
    title: 真实项目验证
    details: 在 mall-portal-front 验证，发现 793 个边界问题，100% 准确率
    link: /packages/webpack-spec-plugin/validation-report
    
  - icon: 📊
    title: HTML 可视化报告
    details: 自动生成详细的 HTML 报告，包含问题说明和修复建议
    
  - icon: ⚡
    title: 增量检查 + 智能缓存
    details: 性能提升 3-10 倍，支持大型项目快速检查
---

## 🚀 快速导航

### Webpack 规范检查插件
强大的代码质量检查工具，集成在 Webpack 构建流程中。

- [📚 插件文档](/packages/webpack-spec-plugin/) - 完整使用文档
- [⚡ 快速开始](/packages/webpack-spec-plugin/quick-start) - 5分钟快速集成
- [🎯 功能特性](/packages/webpack-spec-plugin/features) - 7大检查维度
- [📊 真实项目验证](/packages/webpack-spec-plugin/validation-report) - mall-portal-front 验证报告

### 完整规范文档
涵盖前端开发各个方面的规范文档。

- [📝 规范总览](/specs/) - 查看所有规范
- [💻 编码规范](/specs/coding/) - JS/TS、Vue、CSS、命名、注释等
- [🔧 工程化规范](/specs/engineering/) - Git、Code Review、测试、构建
- [🎯 性能优化](/specs/optimization/) - 性能、内存、资源优化

### 前端工具库
常用的前端工具函数集。

- [🛠️ 工具库文档](/packages/core-utils/) - 完整文档
- [🌐 DOM 操作](/packages/core-utils/dom) - DOM 相关工具
- [✅ 数据校验](/packages/core-utils/validation) - 表单校验工具

### AI 编码助手
提升开发效率和代码质量的AI工具。

- [🤖 AI 使用规范](/ai-guidelines/) - 完整使用规范
- [💡 基础使用技巧](/ai-guidelines/basic-usage) - 快速上手指南
- [🚀 高阶使用技巧](/ai-guidelines/advanced-techniques) - 进阶应用
- [🛡️ 安全使用规范](/ai-guidelines/security-guidelines) - 安全保障

---

## 🎯 核心特性

### Webpack 规范检查插件

**核心亮点**：
- ✅ **7 大检查维度**：命名、注释、边界处理、性能、安全、内存、导入
- ✅ **36 项检查规则**：全面覆盖常见问题
- ✅ **增量检查 + 智能缓存**：性能提升 3-10 倍
- ✅ **HTML 可视化报告**：详细修复方案
- ✅ **真实项目验证**：100% 准确率

**快速开始**：
```bash
npm install @51jbs/webpack-spec-plugin --save-dev
```

[查看完整文档 →](/packages/webpack-spec-plugin/)

---

## 📊 项目统计

- **规范文档**：50+ 份详细文档
- **检查规则**：36 项自动化检查
- **真实案例**：发现 793 个边界问题
- **工具库**：DOM、校验、工具函数