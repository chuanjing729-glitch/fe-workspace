...
function checkUnsafePropertyAccess(filePath: string, fullContent: string, scriptContent: string, localImports: Map<string, string>): CheckResult[] {
  ...
  while ((match = deepAccessPattern.exec(scriptContent)) !== null) {
    ...
    const after = scriptContent.substring(match.index, match.index + fullMatch.length + 2)
    ...
    const hasNullCheck = new RegExp(`if\\s*\\(\\s*${rootObj}\\s*[&|]`, 'm').test(contextBefore) ||
                        new RegExp(`${rootObj}\\s*&&`, 'm').test(contextBefore)
    ...
    if (!hasNullCheck) {
      ...
      results.push({
        ...
        message: `不安全的属性访问："${fullMatch}"，建议使用可选链 "${rootObj}?.${match[2]}?.${match[3]}" 或添加空值检查`,
        file: filePath,
        type: 'error',
        line
      })
