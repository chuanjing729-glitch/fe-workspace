...
class CoverageCollaborationManager {
    ...
    async notifyApiChange(changeInfo) {
        ...
        try {
            ...
            console.log('[ApiTrackerPlugin] Notified webpack-coverage-plugin about API changes');
        }
        catch (error) {
            console.error('[ApiTrackerPlugin] Failed to notify webpack-coverage-plugin:', error);
        }
    }
    isCoveragePluginAvailable() {
        try {
            const coveragePluginPath = path.join(process.cwd(), 'node_modules', 'webpack-coverage-plugin');
