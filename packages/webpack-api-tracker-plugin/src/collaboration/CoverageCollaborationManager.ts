...
/**
 * 覆盖率插件协作管理器
 * 负责与webpack-coverage-plugin的集成和协作
 */
export class CoverageCollaborationManager {
  ...
  /**
   * 通知webpack-coverage-plugin关于API变更
  ...
  async notifyApiChange(changeInfo: any): Promise<void> {
    ...
    try {
      // 将API变更通知写入webpack-coverage-plugin可以读取的文件
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          ...
          changeInfo,
          plugin: 'webpack-api-tracker-plugin'
        }, null, 2)
      );

      console.log('[ApiTrackerPlugin] 已通知webpack-coverage-plugin关于API变更');
    } catch (error) {
      console.error('[ApiTrackerPlugin] 通知webpack-coverage-plugin失败:', error);
    }
  }

  /**
   * 检查webpack-coverage-plugin是否已安装并可用
   * @returns 插件是否可用
   */
