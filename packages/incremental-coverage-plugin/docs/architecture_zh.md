# Incremental Coverage Plugin - 技术架构文档

**版本**: 1.0.0  
**更新时间**: 2025-12-20  
**作者**: chuanjing.li

---

## 📖 目录

1. [系统概述](#系统概述)
2. [核心架构](#核心架构)
3. [模块设计](#模块设计)
4. [数据流](#数据流)
5. [API 规范](#api-规范)
6. [扩展点](#扩展点)
7. [最佳实践](#最佳实践)

---

## 1. 系统概述

### 1.1 设计目标

**核心理念**: 基于业界标准工具，构建轻量、可靠的增量覆盖率插件

**关键目标**:
- ✅ **轻量化**: 代码量 <500 行
- ✅ **标准化**: 使用 babel-plugin-istanbul + istanbul-diff
- ✅ **通用性**: 支持 Webpack/Vite/Rollup 等多种构建工具
- ✅ **可维护**: 模块化设计，职责清晰
- ✅ **易用性**: 零配置即可使用，配置灵活

### 1.2 技术选型

| 技术 | 用途 | 理由 |
|------|------|------|
| **babel-plugin-istanbul** | 代码插桩 | 业界标准，10M+ 月下载量 |
| **istanbul-diff** | 增量计算 | 专门用于增量覆盖率 |
| **unplugin** | 构建工具适配 | 一次编写，多处运行 |
| **simple-git** | Git 集成 | 简单可靠的 Git 操作 |
| **TypeScript** | 类型安全 | 提高代码质量和可维护性 |

### 1.3 架构原则

1. **单一职责**: 每个模块只做一件事
2. **依赖倒置**: 依赖抽象而非具体实现
3. **开闭原则**: 对扩展开放，对修改关闭
4. **最小惊讶**: API 设计符合直觉
5. **渐进增强**: 核心功能简单，高级功能可选

---

## 2. 核心架构

### 2.1 整体架构图

```
┌─────────────────────────────────────────────────────────┐
│                    用户项目                              │
│  ┌──────────────┐         ┌──────────────┐             │
│  │  Webpack     │   或    │    Vite      │             │
│  └──────┬───────┘         └──────┬───────┘             │
│         │                        │                      │
│         └────────────┬───────────┘                      │
│                      │                                   │
│         ┌────────────▼────────────┐                     │
│         │  Incremental Coverage   │                     │
│         │       Plugin            │                     │
│         └────────────┬────────────┘                     │
└──────────────────────┼──────────────────────────────────┘
                       │
        ┌──────────────┼──────────────┐
        │              │              │
┌───────▼──────┐ ┌────▼─────┐ ┌─────▼──────┐
│ Babel Plugin │ │ Coverage │ │   Report   │
│  Istanbul    │ │ Collector│ │  Generator │
└───────┬──────┘ └────┬─────┘ └─────┬──────┘
        │              │              │
        │     ┌────────▼────────┐     │
        │     │  Coverage       │     │
        │     │   Differ        │     │
        │     │ (istanbul-diff) │     │
        │     └────────┬────────┘     │
        │              │              │
        │     ┌────────▼────────┐     │
        │     │   Git Service   │     │
        │     │  (simple-git)   │     │
        │     └─────────────────┘     │
        │                             │
        └─────────────┬───────────────┘
                      │
              ┌───────▼────────┐
              │  HTML / JSON   │
              │    Reports     │
              └────────────────┘
```

### 2.2 分层架构

```
┌─────────────────────────────────────┐
│  Plugin Layer (插件层)               │  ← unplugin 适配层
│  - webpack.ts                        │
│  - vite.ts                           │
└─────────────┬───────────────────────┘
              │
┌─────────────▼───────────────────────┐
│  Core Layer (核心层)                 │  ← 业务逻辑层
│  - plugin.ts (主控制器)              │
│  - collector.ts (数据收集)           │
│  - differ.ts (差异计算)              │
│  - reporter.ts (报告生成)            │
└─────────────┬───────────────────────┘
              │
┌─────────────▼───────────────────────┐
│  Infrastructure Layer (基础设施层)   │  ← 工具和服务
│  - git.ts (Git 操作)                 │
│  - types.ts (类型定义)               │
└─────────────────────────────────────┘
```

---

## 3. 模块设计

### 3.1 Plugin (主控制器)

**职责**: 
- 初始化各个模块
- 协调模块间的交互
- 提供统一的配置接口

**关键方法**:
```typescript
class IncrementalCoveragePlugin {
  // 初始化插件
  constructor(options: IncrementalCoverageOptions)
  
  // Webpack 钩子
  webpack(compiler: Compiler): void
  
  // Vite 钩子
  vite: {
    configResolved(config): void
    configureServer(server): void
  }
}
```

**设计要点**:
- 使用 unplugin 实现跨构建工具兼容
- 配置合并采用浅拷贝 + 用户配置覆盖
- 延迟初始化：只在需要时创建模块实例

### 3.2 CoverageCollector (覆盖率收集器)

**职责**:
- 接收浏览器上报的覆盖率数据
- 合并多次上报的数据
- 维护全局覆盖率状态

**核心算法**:
```typescript
merge(newCoverage: CoverageMap): CoverageMap {
  // 1. 遍历新数据的每个文件
  for (const [file, data] of Object.entries(newCoverage)) {
    if (!this.coverageMap[file]) {
      // 首次出现的文件，直接存储
      this.coverageMap[file] = data;
    } else {
      // 已存在的文件，合并计数
      this.mergeCoverageData(this.coverageMap[file], data);
    }
  }
  return this.coverageMap;
}
```

**合并策略**:
- **语句覆盖**: 计数累加 `s[id] += newS[id]`
- **函数覆盖**: 计数累加 `f[id] += newF[id]`
- **分支覆盖**: 数组元素累加 `b[id][i] += newB[id][i]`

### 3.3 CoverageDiffer (差异计算器)

**职责**:
- 加载 baseline 覆盖率
- 使用 istanbul-diff 计算差异
- 结合 Git diff 获取变更行
- 计算增量覆盖率

**计算流程**:
```
1. 加载 baseline
   ↓
2. istanbul-diff.diff(baseline, current)
   ↓
3. getGitDiff() 获取变更文件和行号
   ↓
4. 对每个变更文件：
   - 获取变更的行号
   - 检查这些行是否被覆盖
   - 计算覆盖率百分比
   ↓
5. 汇总整体覆盖率
   ↓
6. 保存新 baseline（可选）
```

**关键算法**:
```typescript
getUncoveredLines(coverage: CoverageData, changedLines: number[]): number[] {
  const uncovered: number[] = [];
  
  for (const line of changedLines) {
    let isCovered = false;
    
    // 检查是否有语句覆盖这一行
    for (const [stmtId, loc] of Object.entries(coverage.statementMap)) {
      if (loc.start.line <= line && loc.end.line >= line) {
        if (coverage.s[stmtId] > 0) {
          isCovered = true;
          break;
        }
      }
    }
    
    if (!isCovered) {
      uncovered.push(line);
    }
  }
  
  return uncovered;
}
```

### 3.4 CoverageReporter (报告生成器)

**职责**:
- 生成 HTML 报告
- 生成 JSON 报告
- 控制台输出摘要

**报告格式**:

**HTML 报告**:
- 响应式设计
- 渐变色头部
- 卡片式统计
- 文件列表详情

**JSON 报告**:
```json
{
  "overall": {
    "totalLines": 100,
    "coveredLines": 80,
    "coverageRate": 80
  },
  "files": [
    {
      "file": "src/utils.ts",
      "changedLines": [10, 11, 12],
      "uncoveredLines": [11],
      "coverageRate": 66.67
    }
  ],
  "timestamp": 1703001234567
}
```

### 3.5 GitService (Git 服务)

**职责**:
- 获取 Git diff
- 解析 diff 输出
- 提取变更的行号

**Diff 解析算法**:
```typescript
parseDiff(diffText: string): { additions: number[], deletions: number[] } {
  // 1. 按行分割
  const lines = diffText.split('\n');
  
  // 2. 查找 hunk header (@@ -old +new @@)
  // 3. 解析起始行号
  // 4. 遍历每一行：
  //    - '+' 开头 → 新增行
  //    - '-' 开头 → 删除行
  //    - ' ' 开头 → 上下文行
  // 5. 返回行号数组
}
```

---

## 4. 数据流

### 4.1 完整数据流图

```
┌──────────────┐
│  用户修改代码  │
└──────┬───────┘
       │
       ▼
┌──────────────────────┐
│  Webpack/Vite 构建    │
│  + babel-plugin-      │
│    istanbul 插桩      │
└──────┬───────────────┘
       │
       ▼
┌──────────────────────┐
│  浏览器执行代码       │
│  收集 window.        │
│  __coverage__        │
└──────┬───────────────┘
       │
       ▼ (POST /coverage)
┌──────────────────────┐
│  HTTP 中间件         │
│  接收覆盖率数据       │
└──────┬───────────────┘
       │
       ▼
┌──────────────────────┐
│  CoverageCollector   │
│  合并覆盖率数据       │
└──────┬───────────────┘
       │
       ▼
┌──────────────────────┐
│  CoverageDiffer      │
│  1. 加载 baseline    │
│  2. istanbul-diff    │
│  3. Git diff         │
│  4. 计算增量         │
└──────┬───────────────┘
       │
       ▼
┌──────────────────────┐
│  CoverageReporter    │
│  生成 HTML/JSON      │
└──────┬───────────────┘
       │
       ▼
┌──────────────────────┐
│  输出报告            │
│  - .coverage/        │
│    latest.html       │
│  - 控制台摘要         │
└──────────────────────┘
```

### 4.2 关键数据结构

**CoverageMap** (Istanbul 标准格式):
```typescript
{
  "src/utils.ts": {
    "path": "src/utils.ts",
    "statementMap": {
      "0": { "start": { "line": 1, "column": 0 }, "end": { "line": 1, "column": 20 } }
    },
    "fnMap": { ... },
    "branchMap": { ... },
    "s": { "0": 5 },  // 语句执行次数
    "f": { "0": 3 },  // 函数执行次数
    "b": { "0": [2, 1] }  // 分支执行次数
  }
}
```

**IncrementalCoverageResult**:
```typescript
{
  "overall": {
    "totalLines": 100,
    "coveredLines": 80,
    "coverageRate": 80
  },
  "files": [
    {
      "file": "src/utils.ts",
      "changedLines": [10, 11, 12],
      "uncoveredLines": [11],
      "coverageRate": 66.67
    }
  ],
  "changedFiles": ["src/utils.ts", "src/helper.ts"],
  "timestamp": 1703001234567
}
```

---

## 5. API 规范

### 5.1 配置 API

```typescript
interface IncrementalCoverageOptions {
  // 文件包含模式（glob）
  include?: string[];  // 默认: ['src/**']
  
  // 文件排除模式（glob）
  exclude?: string[];  // 默认: ['**/*.spec.ts', '**/*.test.ts']
  
  // Git 对比基准分支
  gitDiffBase?: string;  // 默认: 'main'
  
  // 报告输出目录
  outputDir?: string;  // 默认: '.coverage'
  
  // 报告格式
  reportFormat?: 'html' | 'json' | 'both';  // 默认: 'html'
  
  // 启用浏览器 UI
  enableOverlay?: boolean;  // 默认: true
  
  // Baseline 文件路径
  baselinePath?: string;  // 默认: '.coverage/baseline.json'
  
  // 自动保存 baseline
  autoSaveBaseline?: boolean;  // 默认: true
  
  // 覆盖率阈值
  threshold?: number;  // 默认: 80
}
```

### 5.2 使用示例

**Webpack**:
```javascript
const { WebpackIncrementalCoveragePlugin } = require('@51jbs/incremental-coverage-plugin/webpack');

module.exports = {
  plugins: [
    new WebpackIncrementalCoveragePlugin({
      include: ['src/**'],
      gitDiffBase: 'main',
      threshold: 80,
    })
  ]
};
```

**Vite**:
```javascript
import { ViteIncrementalCoveragePlugin } from '@51jbs/incremental-coverage-plugin/vite';

export default {
  plugins: [
    ViteIncrementalCoveragePlugin({
      include: ['src/**'],
      gitDiffBase: 'develop',
    })
  ]
};
```

---

## 6. 扩展点

### 6.1 自定义报告格式

```typescript
class CustomReporter extends CoverageReporter {
  async generate(result: IncrementalCoverageResult): Promise<void> {
    // 自定义报告逻辑
    await this.generateMarkdown(result);
  }
  
  private async generateMarkdown(result: IncrementalCoverageResult): Promise<void> {
    // 生成 Markdown 格式报告
  }
}
```

### 6.2 自定义 Diff 策略

```typescript
class CustomDiffer extends CoverageDiffer {
  async calculate(coverage: CoverageMap): Promise<IncrementalCoverageResult> {
    // 自定义差异计算逻辑
    // 例如：只关注特定目录的变更
  }
}
```

### 6.3 插件钩子

```typescript
// 未来可以添加的钩子
interface PluginHooks {
  onCoverageCollected?: (coverage: CoverageMap) => void;
  onDiffCalculated?: (result: IncrementalCoverageResult) => void;
  onReportGenerated?: (reportPath: string) => void;
}
```

---

## 7. 最佳实践

### 7.1 性能优化

1. **延迟加载**: 只在需要时加载模块
2. **缓存策略**: 缓存 Git diff 结果
3. **防抖处理**: 避免频繁生成报告
4. **增量更新**: 只处理变更的文件

### 7.2 错误处理

```typescript
try {
  const baseline = this.loadBaseline();
} catch (error) {
  console.warn('[CoverageDiffer] 无法加载 baseline:', error);
  // 使用空 baseline 继续
  return this.createEmptyBaseline();
}
```

### 7.3 日志规范

```typescript
// 使用统一的日志前缀
console.log('[PluginName] 操作描述');
console.warn('[PluginName] 警告信息');
console.error('[PluginName] 错误信息');
```

### 7.4 测试策略

1. **单元测试**: 测试每个模块的核心逻辑
2. **集成测试**: 测试模块间的协作
3. **端到端测试**: 测试完整的工作流

---

## 8. 设计决策记录

### 8.1 为什么选择 unplugin？

**决策**: 使用 unplugin 而不是分别实现 webpack/vite 插件

**理由**:
- ✅ 一次编写，多处运行
- ✅ 减少重复代码
- ✅ 统一的 API 设计
- ✅ 社区支持好

**权衡**:
- ❌ 增加一个依赖
- ❌ 学习成本稍高

### 8.2 为什么使用 istanbul-diff？

**决策**: 使用 istanbul-diff 而不是自己实现

**理由**:
- ✅ 经过验证的算法
- ✅ 处理边界情况
- ✅ 标准化输出
- ✅ 减少维护成本

**权衡**:
- ❌ 依赖外部库
- ❌ 输出格式固定

### 8.3 为什么分离 Collector 和 Differ？

**决策**: 将数据收集和差异计算分离

**理由**:
- ✅ 单一职责原则
- ✅ 易于测试
- ✅ 可独立替换
- ✅ 逻辑清晰

---

## 9. 未来规划

### 9.1 短期（v1.1）
- [ ] 浏览器 overlay UI
- [ ] 实时覆盖率更新
- [ ] 更丰富的报告样式

### 9.2 中期（v1.2）
- [ ] 支持更多构建工具（Rollup, esbuild）
- [ ] 覆盖率趋势图
- [ ] CI/CD 集成

### 9.3 长期（v2.0）
- [ ] 云端服务
- [ ] 团队协作
- [ ] 智能测试推荐

---

## 10. 参考资料

- [babel-plugin-istanbul](https://github.com/istanbuljs/babel-plugin-istanbul)
- [istanbul-diff](https://github.com/istanbuljs/istanbul-diff)
- [unplugin](https://github.com/unjs/unplugin)
- [Istanbul Coverage Format](https://github.com/istanbuljs/istanbuljs/blob/master/packages/istanbul-lib-coverage/lib/file-coverage.js)

---

**文档版本**: 1.0.0  
**最后更新**: 2025-12-20  
**维护者**: chuanjing729
